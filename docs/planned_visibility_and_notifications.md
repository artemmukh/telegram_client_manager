...
Статус: бэклог. Пункты 1.1, 1.2, 2.1, 2.2 и 2.3 закрыты в текущей партии; остальные
пункты остаются результатом анализа, а не одобренным планом работ. Приоритеты —
рекомендация, а не решение.

Все фазы идут через обязательный workflow проекта (`.codex/agents/workflow.md`):
...
Для зарегистрированных пользователей источником прав доступа является `staff`:
`AuthService` определяет роль по нему, а `RoleFilter` и `UserContextMiddleware`
опираются на этот путь. `users.role` остается только совместимым хранимым кэшем и
не backfill'ится и не синхронизируется.

Телеграм-ID, который есть только в `staff`, остается незарегистрированным до
фактической регистрации. При перерисовке профиля сохраняется роль, которую выдал
middleware, и одновременно применяются возвращенные настройки языка.

"""Handler-level tests for appointment_browser.py's calendar+slot-picker
edit-datetime flow (Phase D): replaces the old free-text "new datetime"
prompt used by start_edit_datetime with the same month-grid calendar +
slot-occupancy grid used by the Phase C creation flow and mirrored in
booking_requests.py/reschedule_requests.py's propose flows (see
test_booking_requests_calendar_slot_picker.py /
test_reschedule_requests_calendar_slot_picker.py), scoped to
AppointmentBrowserStates.edit_choose_day/edit_choose_slot.

Covers: start_edit_datetime's branch on DATA_PARSE_MODE.get(instance),
pick_edit_datetime_calendar_day (including the exclude_appointment_id
behavior new to this flow -- the appointment's own current slot must not
render as occupied), pick_edit_datetime_slot, the back-navigation chain, and
an integration test proving the untouched approve_new_datetime handler still
works correctly on the FSM data the new slot picker produces.

Follows the direct-handler-call/fake-repository convention established in
test_appointment_browser_propose_handler.py (in particular its
copy_on_read=True FakeAppointmentRepository option, needed here too since
approve_new_datetime reads the appointment's pre-mutation datetime for the
"old time" display after propose_new_datetime has already mutated it).
"""
...
from bot.keyboards.admin.record_management_kb.appointment_browser_cb import ApptActionCB, ApptCalendarDayCB
from bot.keyboards.admin.record_management_kb.appointment_browser_kb import appointment_browser_cancel_edit_kb
...
NOW_PATCH_TARGET = "bot.handlers.admin.appointment_management.appointment_browser.get_current_tashkent_datetime"


class FakeAppointmentRepository:
    def __init__(self, appointment, other_appointments=None, copy_on_read=False):
...
# --- start_edit_datetime: branch on DATA_PARSE_MODE ---

@pytest.mark.asyncio
async def test_start_edit_datetime_slots_mode_enters_calendar_and_stashes_staff_and_old_datetime():
    appointment = _confirmed_appointment()
    router = _build_router(FakeAppointmentRepository(appointment))
    start_edit_datetime = _find_handler(router, "start_edit_datetime")
...
@pytest.mark.asyncio
async def test_start_edit_datetime_missing_appointment_shows_alert_and_does_not_enter_calendar():
    appointment = _confirmed_appointment()
    router = _build_router(FakeAppointmentRepository(appointment))
    start_edit_datetime = _find_handler(router, "start_edit_datetime")
...
@pytest.mark.asyncio
async def test_start_edit_datetime_non_slots_mode_still_uses_free_text_prompt():
    appointment = _confirmed_appointment()
    ...
    callback_query.message.edit_text.assert_awaited_once_with(
        ...
        reply_markup=appointment_browser_cancel_edit_kb(1, "list", 1),
    )


# --- pick_edit_datetime_calendar_day ---

@pytest.mark.asyncio
async def test_pick_edit_datetime_calendar_day_excludes_the_appointments_own_current_slot_from_occupied_markers():
    day_iso = "2026-08-15"
    ...
    pick_edit_datetime_calendar_day = _find_handler(router, "pick_edit_datetime_calendar_day")

    callback_query = _callback_query()
    state = _state(appointment_id=1, mode="list", page=1, staff_user_id=DOCTOR_ID, old_datetime=f"{day_iso} 09:00")
...
@pytest.mark.asyncio
async def test_back_to_edit_datetime_day_selection_returns_to_month_grid_not_full_reset():
    appointment = _confirmed_appointment()
    router = _build_router(FakeAppointmentRepository(appointment))
    back_to_edit_datetime_day_selection = _find_handler(router, "back_to_edit_datetime_day_selection")
...
@pytest.mark.asyncio
async def test_view_edit_datetime_occupied_slot_then_back_returns_to_slot_grid_not_full_reset():
    day_iso = "2026-08-15"
    appointment = _confirmed_appointment(day_iso=day_iso, slot="09:00", appointment_id=1)
    ...
    router = _build_router(appt_repo)
    view_edit_datetime_occupied_slot = _find_handler(router, "view_edit_datetime_occupied_slot")
    back_to_edit_datetime_slot_grid = _find_handler(router, "back_to_edit_datetime_slot_grid")
...
# --- pick_edit_datetime_slot ---

@pytest.mark.asyncio
async def test_pick_edit_datetime_slot_writes_parsed_datetime_and_display_then_moves_to_confirm_state():
    day_iso = "2026-08-15"
    appointment = _confirmed_appointment(day_iso=day_iso, slot="09:00", appointment_id=1)
    router = _build_router(FakeAppointmentRepository(appointment))
    pick_edit_datetime_slot = _find_handler(router, "pick_edit_datetime_slot")
    ...
    state.set_state.assert_awaited_once_with(AppointmentBrowserStates.confirm_new_datetime)

    old_display = format_datetime_for_confirmation(datetime(2026, 8, 15, 9, 0))
    from bot.keyboards.admin.record_management_kb.appointment_browser_kb import appointment_confirm_new_datetime_kb
    callback_query.message.edit_text.assert_awaited_once_with(
        f"Новая дата и время: {old_display} → {expected_display}",
        reply_markup=appointment_confirm_new_datetime_kb(1, "list", 1),
    )
    callback_query.answer.assert_awaited_once()


@pytest.mark.asyncio
async def test_pick_edit_datetime_slot_with_malformed_slot_shows_alert_and_does_not_touch_state():
    appointment = _confirmed_appointment()
    router = _build_router(FakeAppointmentRepository(appointment))
    pick_edit_datetime_slot = _find_handler(router, "pick_edit_datetime_slot")
...
# --- integration: new entry path -> untouched approve_new_datetime exit path ---

@pytest.mark.asyncio
async def test_pick_edit_datetime_slot_then_approve_new_datetime_commits_the_picked_slot():
    day_iso = "2026-08-15"
    appointment = _confirmed_appointment(day_iso=day_iso, slot="09:00", appointment_id=1)
    ...
    await state.update_data(
        appointment_id=1, mode="list", page=1, day_iso=day_iso, old_datetime=f"{day_iso} 09:00",
    )
    ...
    assert resynced.datetime == "2026-08-15 11:00"

"""Card-level ownership check tests for reschedule_requests.py.

docs/appointment_card_ownership_check_prompt.md: accept/reject/cancel_propose/
...
def _foreign_reschedule_request(status=AppointmentStatus.CONFIRMED):
    """Same clinic as the own-scope admin, but a different doctor. Carries a
    client counter-proposal so accept/reject have something to act on."""
    return Appointment(
        clinic_id=1,
        client_id=7,
...
def _other_clinic_reschedule_request(status=AppointmentStatus.CONFIRMED):
    return Appointment(
        clinic_id=2,
        client_id=7,
        doctor_id=OTHER_DOCTOR_ID,
...
@pytest.mark.asyncio
async def test_accept_reschedule_denies_own_scope_admin_for_other_doctor_appointment():
    appt_repo = FakeAppointmentRepository([_foreign_reschedule_request()])
...
@pytest.mark.asyncio
async def test_accept_reschedule_denies_clinic_scope_admin_for_other_clinic_appointment():
    appt_repo = FakeAppointmentRepository([_other_clinic_reschedule_request()])
    router = _build_router(appt_repo)
    accept_reschedule = _find_handler(router, "accept_reschedule")
...
@pytest.mark.asyncio
async def test_accept_reschedule_foreign_and_nonexistent_ids_give_identical_response():
    appt_repo = FakeAppointmentRepository([_foreign_reschedule_request()])
    router = _build_router(appt_repo)
    accept_reschedule = _find_handler(router, "accept_reschedule")
...
@pytest.mark.asyncio
async def test_accept_reschedule_denies_when_admin_user_missing():
    appt_repo = FakeAppointmentRepository([_foreign_reschedule_request()])
    router = _build_router(appt_repo)
    accept_reschedule = _find_handler(router, "accept_reschedule")
...
@pytest.mark.asyncio
async def test_reject_reschedule_denies_own_scope_admin_for_other_doctor_appointment():
    appt_repo = FakeAppointmentRepository([_foreign_reschedule_request()])
    router = _build_router(appt_repo)

"""Handler-level tests for booking_requests.py's calendar+slot-picker propose
flow (Phase D): replaces the old free-text "propose new datetime" prompt with
the same month-grid calendar + slot-occupancy grid used by the Phase C
creation flow (bot/handlers/admin/appointment_management/appointment_creation.py),
scoped to BookingNegotiationStates.choose_day/choose_slot.
...
NOW_PATCH_TARGET = "bot.handlers.admin.appointment_management.booking_requests.get_current_tashkent_datetime"
...
def _pending_appointment(day_iso="2026-08-15", slot="09:00", appointment_id=1):
    return Appointment(
        clinic_id=1,
        client_id=7,
        doctor_id=DOCTOR_ID,
...
async def test_start_propose_datetime_slots_mode_enters_calendar_and_stashes_staff_and_old_datetime():
    appointment = _pending_appointment()
...
async def test_start_propose_datetime_non_slots_mode_still_uses_free_text_prompt():
    ...
    callback_query.message.edit_text.assert_awaited_once_with(
        ...
        reply_markup=booking_request_propose_cancel_kb(1),
    )
...
async def test_pick_propose_calendar_day_excludes_the_appointments_own_current_slot_from_occupied_markers():
    day_iso = "2026-08-15"
    appointment = _pending_appointment(day_iso=day_iso, slot="09:00", appointment_id=1)
...
async def test_pick_propose_slot_writes_parsed_datetime_and_display_then_moves_to_confirm_state():
    ...
    state.set_state.assert_awaited_once_with(BookingNegotiationStates.confirm_propose_datetime)
    ...
    callback_query.message.edit_text.assert_awaited_once_with(
        ...
        reply_markup=booking_request_confirm_propose_kb(1),
    )
    callback_query.answer.assert_awaited_once()

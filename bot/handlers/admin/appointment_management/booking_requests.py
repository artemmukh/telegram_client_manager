...
def create_admin_booking_requests_router(
    ...
    async def change_propose_calendar_month(
        ...
        data = await state.get_data()
        back_callback_data, back_label = ah.propose_calendar_back_target(BookingRequestActionCB, data["appointment_id"])
    ...
    async def pick_propose_slot(callback_query: CallbackQuery, callback_data: ClientBookSlotCB, state: FSMContext):
        await ah.apply_picked_propose_slot(
            ...
            confirm_kb_builder=booking_request_confirm_propose_kb,
        )

...
def create_admin_appointment_update_router(appointment_repo, user_repo, staff_repo, clinic_repo):
    router = Router()
    ...
    async def change_status(callback_query: CallbackQuery, state: FSMContext):
        ...
        try:
            appointment = await appt_mng.update_status(
                int(appointment_id), AppointmentStatus(status_value)
            )
        except BotException as e:
    ...
    async def process_new_datetime(message: Message, state: FSMContext):
        ...
        try:
            appointment = await appt_mng.update_datetime(data["appointment_id"], message.text.strip())
        except ValidationError as e:
            await message.answer(str(e))
            return
        except BotException as e:

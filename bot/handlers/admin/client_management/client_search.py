...
from bot.exceptions.user_exceptions import InvalidPhoneError, InvalidFullNameError, ValidationError, UserNotFoundError
...
def create_admin_client_search_router(user_repo):
    ...
    async def client_search(callback_query: CallbackQuery, state: FSMContext):
    ...
    async def process_client_full_name_search(message: Message, state: FSMContext):
        if not await full_name_processing(message, state, next_state=ClientSearchStates.confirm_search):
            return
    ...
    async def process_client_phone_search(message: Message, state: FSMContext):
        if not await phone_processing(message, state, next_state=ClientSearchStates.confirm_search):
            return
    ...
    async def process_full_name_edition(message: Message, state: FSMContext):
        if not await process_edit_full_name(
                ...
                reply_markup=client_search_name_kb(),
        ):
            return
    ...
    async def process_phone_edition(message: Message, state: FSMContext):
        if not await process_edit_phone(
                message, state,
                final_state=ClientSearchStates.confirm_search,
                reply_markup=client_search_phone_kb(),
        ):
            return

    @router.callback_query(F.data == "get_all_clients")
    async def get_all_clients(message: Message, state: FSMContext):
        await phone_processing(message, state, next_state=ClientSearchStates.confirm_search)
    ...
    async def client_creation_finish(callback_query: CallbackQuery, state: FSMContext):
        ...
        try:
            found_user = await cl_mng.search_client(data)

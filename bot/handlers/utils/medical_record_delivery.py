...
async def deliver_medical_record(
    ...
    Ready documents are sent to the caller's own chat via answer_document; any
    other status (pending/generating/failed) gets a polite "check back later"
    alert instead. Callers are responsible for verifying the caller owns/may
    view this appointment before calling this helper.
    """
